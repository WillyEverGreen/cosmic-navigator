...
const MissionTimeline = () => {
  ...
  const getStatusStyle = (status: Mission['status']) => {
    switch (status) {
      case 'completed': return 'bg-muted/30 border-muted-foreground/30 opacity-60';
      case 'active': return 'bg-cosmic-glow/20 border-cosmic-glow glow-pulse';
      case 'upcoming': return 'bg-muted/20 border-muted/50';
    }
  };
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cosmic-glow/10 border border-cosmic-glow/20 mb-4">
            ...
            <span className="text-sm text-cosmic-glow font-medium">Mission Timeline</span>
          </div>
            ...
            {missions.map((mission, index) => (
              ...
              >
                ...
                <div className="flex flex-col items-center mb-4">
                  <motion.div
                    animate={mission.status === 'active' ? { scale: [1, 1.2, 1] } : {}}
                    ...
                    style={mission.status === 'active' ? { boxShadow: '0 0 20px rgba(99, 102, 241, 0.6)' } : {}}
                  />
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    <div className={`p-1.5 rounded-lg ${
                      mission.status === 'active' ? 'bg-cosmic-glow/20 text-cosmic-glow' : 'bg-muted text-muted-foreground'
                    }`}>
                  ...
                  >
                    <div className="pt-4 border-t border-border/50">
                      <p className="text-sm text-muted-foreground mb-2">Impact on Earth science:</p>
                      <p className="text-sm text-star-white">{mission.impact}</p>
                    </div>
                  </motion.div>
                  ...
                  <div className="flex items-center justify-between mt-3">
                    <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                      ...
                      mission.status === 'active' ? 'bg-cosmic-glow/20 text-cosmic-glow' :
                      'bg-cosmic-purple/20 text-cosmic-purple'
                    }`}>
                      {mission.status.charAt(0).toUpperCase() + mission.status.slice(1)}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      {expandedMission === mission.id ? 'Click to collapse' : 'Click for details'}
                    </span>
                  </div>
                </motion.button>
              </motion.div>
